...
func (h *Hostfile) Parse() []error {
	var errs []error
	var line = 1
	for v := range strings.SplitSeq(string(h.data), "\n") {
...
func (h *Hostfile) Save() error {
	...
	if runtime.GOOS == "windows" {
		...
		file, err = os.OpenFile(h.Path, os.O_RDWR, 0o644)
	} else {
		...
		// from history.
		file, err = os.OpenFile(h.Path, os.O_RDWR|os.O_APPEND|os.O_TRUNC, 0o644)
	}
	if err != nil {
		return err
	}
